...
void EnemyManager::update()
{
	...
	for (_viEnemy = _vEnemy.begin(); _viEnemy != _vEnemy.end(); ++_viEnemy)
	{
		...
		(*_viEnemy)->setTileInfo(
			_map->getMapInfo((*_viEnemy)->getPoint().y % TILESIZE - 1, (*_viEnemy)->getPoint().x %TILESIZE),
			_map->getMapInfo((*_viEnemy)->getPoint().y % TILESIZE + 1, (*_viEnemy)->getPoint().x %TILESIZE),
			_map->getMapInfo((*_viEnemy)->getPoint().y % TILESIZE, (*_viEnemy)->getPoint().x %TILESIZE - 1),
			_map->getMapInfo((*_viEnemy)->getPoint().y % TILESIZE, (*_viEnemy)->getPoint().x %TILESIZE + 1));
